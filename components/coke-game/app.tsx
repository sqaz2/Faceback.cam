...
import { MixerPanel } from "@/components/coke-game/mixer-panel";
import { UncoverMusic, VegaSan } from "@/components/coke-game/minigames";
import { Wordmark } from "@/components/coke-game/wordmark";
import { WorldView } from "@/components/coke-game/world-view";
import {
  ACCESSORIES,
  BODY_STYLES,
  BOTTOM_STYLES,
          ...
          <section key={section.id} id={`catalog-aisle-${section.id}`}>
            ...
            <ul className="grid gap-2">
              {items.map((c) => {
                ...
                const facing = furnitureFacing(rot, c.rotate);
                const thumb = c.sprite ? furnitureSpriteUrl(c.sprite, facing) : undefined;
                return (
                  <li
                    key={c.id}
                  ...
                  >
                    <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-[12px] border border-border/60 bg-ink sm:h-20 sm:w-20">
                      {thumb ? (
                        ...
                        <div
                          className="flex h-full w-full items-center justify-center bg-[radial-gradient(ellipse_at_center,#6e0a12_0%,#2a1216_70%)]"
                          aria-hidden
                        >
