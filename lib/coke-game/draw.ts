...
function pickFurnitureImage(
  sprites: SpriteMap,
  sprite: string,
  rot: number,
  rotate?: "90" | "360",
): HTMLImageElement | undefined {
  const facing = furnitureFacing(rot, rotate);
  const keys = [`${sprite}_d${facing}`, `${sprite}_d0`, sprite];
  for (const k of keys) {
    const img = sprites[k];
...
/** Draw furniture upright using facing PNG (d0–d3). Never canvas-rotates the sprite. */
...
export function renderWorld(
  ...
  for (const it of list) {
    if (it.kind === "furn") {
      ...
      const spr = cat?.sprite
        ? pickFurnitureImage(sprites, cat.sprite, it.item.rot ?? 0, cat.rotate)
        : undefined;
