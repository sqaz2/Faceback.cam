...
function drawSpriteItem(ctx: CanvasRenderingContext2D, item: PlacedItem, img: HTMLImageElement) {
  const cat = CATALOG_MAP[item.catalogId]!;
  const s = tileToScreen(item.x + (cat.w - 1) * 0.5, item.y + (cat.d - 1) * 0.5);
  // Plant the sprite on the tile diamond; groundBias pulls opaque feet onto the tile.
  const footY = s.y + TILE_H * 0.5 + (cat.groundBias ?? 2);
...
export function drawAppearance(
  ...
  try {
    ...
    if (frame) {
      ...
      // Lower foot offset nests sitters onto cushions instead of hovering above.
      const foot = sitting ? 3 : 10;
...
export function renderAvatarPreview(
  ...
  opts?: { scale?: number; dir?: 0 | 1 | 2 | 3; bg?: string },
) {
  ensureAvatarSheets();
  ...
  ctx.fillStyle = opts?.bg ?? "#1f1013";
  ctx.fillRect(0, 0, w, h);
  const dir = opts?.dir ?? ((Math.floor(t / 1.8) % 4) as 0 | 1 | 2 | 3);
...
/** Compact wardrobe option tile — larger chibi, fixed facing, no rotation. */
export function renderOptionPreview(
  canvas: HTMLCanvasElement,
  appearance: Appearance,
  t = 0,
) {
  renderAvatarPreview(canvas, appearance, t, "idle", { scale: 1.85, dir: 1, bg: "#241014" });
}
